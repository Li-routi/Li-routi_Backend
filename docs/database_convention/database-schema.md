...
인증은 당일 것만 삭제 후 재등록할 수 있다. 어제 이전의 인증은 수정할 수 없으므로 연속 참여일을 소급해 재계산하는 경우는 없다. 집안일의 `canceled_at`에 해당하는 컬럼을 두지 않는 이유다.
...
- 당일 인증만 삭제 후 재등록할 수 있다. 삭제는 `deleted_at`으로 처리하되, 유니크 제약이 소프트 삭제된 행까지 포함한다는 점에 유의한다. 재등록을 허용하려면 삭제된 행을 물리 삭제하거나 유니크 제약 설계를 조정해야 한다. **구현 시 이 지점을 반드시 확인한다.**
...
- `challenge_verification`의 소프트 삭제와 `UNIQUE(member_challenge_id, verified_date)`는 충돌할 수 있다. 당일 인증을 삭제한 뒤 다시 등록하려면 삭제된 행이 유니크 제약에 남아 있어 실패한다. 물리 삭제로 처리할지, 유니크 제약에 삭제 여부를 포함할지 구현 전에 정한다.
