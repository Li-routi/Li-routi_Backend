...
- 자기 인증을 신고하는 것을 막지 않는다. 기획에 그런 제약이 없고, 결과가 "본인 피드에서 본인 사진이 안 보인다"뿐이라 해가 없다.
...
동시성은 문제되지 않는다. 두 신고가 동시에 임계값을 넘겨 `hidden_at` 을 두 번 써도 결과가 같다. 반대로 상대의 미커밋 INSERT 를 못 봐 숨김이 한 박자 늦어질 수는 있으나 다음 신고에서 걸린다.
...
**임계값은 설정값이다**(`challenge.report.hide-threshold`, 기본 3). 실사용자 수에 따라 적정값이 달라지고, 오작동 시 값을 올려 되돌리는 탈출구도 된다. 복구 수단은 이것으로 갈음한다 — 관리자 API 는 이 임시 조치의 범위를 넘는다.
