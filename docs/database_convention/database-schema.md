...
- `avatar_purchase_id` 로 **어느 구매에서 왔는지** 가리킨다(아래). 이 표가 생기기 전에 산 행은 `NULL` 이다.
...
| idempotency_key | VARCHAR(150) | N | 클라이언트가 준 값 그대로 |
| item_fingerprint | CHAR(64) | N | **정렬한** `itemIds` 의 해시 |
...
**정렬해서 해시한다.** 같은 아이템을 다른 순서로 보낸 것은 같은 구매다 — 응답의 아이템 순서만 요청 순서를 지킨다.
...
WHERE member_id = :memberId
  AND all_completed = 1
  AND activity_date > :todayKst - INTERVAL 15 DAY
```
