...
**셋은 한 트랜잭션이다.** 나뉘면 재화만 빠지고 아이템이 안 생기거나, 샀는데 안 입혀진 상태가 남는다.
...
### 착용 저장은 보유한 것 사이의 교체다

```text
PUT /api/members/me/avatar     보유한 아이템으로 슬롯을 채운다
```

구매를 거치지 않고 **이미 가진 것끼리 갈아입는** 경로다. 서버는 두 가지를 확인한다.

- **보유한 아이템인가** — 아니면 거절한다. 미보유 착용은 구매를 건너뛰는 길이 된다
- **그 아이템의 슬롯이 맞는가** — 요청이 슬롯을 지정하더라도 아이템이 가진 슬롯을 따른다

**슬롯을 비우는 것도 저장이다.** 벗으면 그 슬롯의 행을 지운다.
