...
> 아래 [기획 확인이 필요한 것]이 정해져야 착수할 수 있다.
...
**그룹 루틴이 가장 까다롭다.** 이미 시간대 제약이 있는 상태 기계가 도는데(스케줄된 시간에만 완료 가능, 지나면 스케줄러가 `MISSED`), 챌린지는 하루 중 아무 때나 + 재인증 덮어쓰기다. **모델이 다르다.**

1. **그룹 루틴 인증에 사진이 필수인가?** 필수라면 기존 완료 처리 경로가 사진 없이 완료시키는 우회로가 된다.
2. **시간대 제약을 유지하는가?** 챌린지처럼 하루 중 아무 때나 허용하면 `status` 전이 모델과 어긋난다.
3. **이미 완료된 것을 재인증할 수 있는가?** 챌린지는 덮어쓰기를 허용하지만 그룹은 지금 409다.
4. **개인 루틴은 요일 스케줄에 없는 날도 인증할 수 있는가?** 예를 들어 월·수·금 루틴을 화요일에 수행했다면.
5. **그룹·개인 인증 사진에도 유해성 심사를 붙이는가?** 아래 [유해성 심사는 챌린지만이 아닐 수 있다] 참고. 미션 부합 심사와 별개 질문이다.
6. **그룹 인증에 신고 기능이 필요한가?** 유해성 심사도 신고도 없으면 방에 부적절한 사진이 올라왔을 때 대응 수단이 없다.
7. **루틴 인증은 무엇을 갱신하는가?** 루틴에는 스트릭 컬럼이 없다. 기록만 남기면 되는지, 무언가 올라야 하는지.

### 유해성 심사는 챌린지만이 아닐 수 있다

기획은 **"AI 심사는 챌린지 전용"**이라고 정했다. 그런데 그 결정 이후 AI 심사가 **두 가지**로 갈렸다.

| 기준 | 내용 | 그룹·개인에 필요한가 |
| --- | --- | --- |
| 미션 부합 | "이 사진이 그 챌린지에 맞나" | **불필요.** 기획 확정대로 챌린지 전용 |
| 공개 가능 여부 | 선정적·폭력적·타인 개인정보 | **재확인 필요** |

**그룹 루틴 인증 사진은 그 방 멤버 전원이 본다.** 방에 유해물을 올리는 것을 막을 수단이 지금 설계에는 없다. 개인 루틴은 본인만 보므로 덜 급하지만 0은 아니다.

미션 부합 심사를 뺀 근거("루틴은 자유로운 수행이라 판정 기준이 없다")가 **유해성에는 그대로 적용되지 않는다.** 둘을 한 덩어리로 결정했다면 다시 나눠 볼 필요가 있다.

### 신고 경로가 없다

`challenge_verification_report`·`challenge_verification_like`는 **챌린지 전용 테이블**이다. 그룹·개인 인증에는 신고도 좋아요도 없다.

좋아요는 없어도 되지만 **신고는 다르다.** 그룹 방에 부적절한 사진이 올라왔을 때 멤버가 할 수 있는 것이 없어진다. AI 유해성 심사를 안 붙이기로 하면 신고라도 있어야 하고, 둘 다 없으면 **대응 수단이 아예 없다.**

### 인증해도 오르는 것이 없다

`member_routine`·`group_routine` 어디에도 스트릭 컬럼이 없다. 챌린지는 `member_challenge.current_streak`가 있어 인증이 스트릭을 올리지만, 루틴 인증은 **무엇을 갱신하는지가 정해져 있지 않다.**

- 그룹은 `group_routine_assignment.status`가 그 역할을 한다고 볼 수 있다
- 개인 루틴은 인증 행이 쌓이는 것 말고 아무 일도 일어나지 않는다

인증의 목적이 "기록을 남기는 것"인지 "무언가를 올리는 것"인지 확인이 필요하다.
