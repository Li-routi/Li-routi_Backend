...
### 심사가 장애로 답을 못 주면 보류한다 — 설계안 (합의 필요)

**지금은 심사기가 답을 못 주면 그냥 통과시킨다.** 이 fail-open 은 근거가 있는 선택이었다 — *외부 API 하나가 인증 기능 전체를 멈추게 두지 않는다*. 문제는 **선택지가 "전부 통과"와 "전부 차단"뿐**이었다는 것이다.
...
| 상황 | 처리 | 왜 |
| --- | --- | --- |
| 심사기 장애·타임아웃 | **보류** | 복구되면 판정이 달라진다 |
| 사진을 못 읽음(S3 일시 오류) | **보류** | 같은 이유 |
| `ai.review.enabled = false` | **통과** | 일부러 끈 것이다. 보류로 두면 킬 스위치가 인증을 막는 스위치가 된다 |
| 챌린지를 못 찾음 | **통과** | 다시 해도 같다. 저장 단계에서 어차피 걸린다 |

**킬 스위치를 가르는 것이 특히 중요하다.** 지금은 이 넷이 모두 `decided = false` 로 같은 자리에 떨어진다.
...
```text
보류 상한        24시간
수명 주기         3일        ← 상한보다 넉넉히 길게
```

상한에 닿으면 아래 [유예가 끝나면 통과시킨다] 대로 처리하므로, **하루를 넘겨 살아 있어야 하는 대기본은 존재하지 않는다.** 수명 주기는 지금처럼 나이 기반 한 줄이면 된다.
...
그래서 **올린다.** 반려 확정 시 `last_verified_date` · `current_streak` 을 보정한다.
...
**`#103` 1단계에서 만든 presigned GET 을 그대로 쓴다.** 보류 중에는 본인용 서명 주소를, 승격 후에는 공개 URL 을 내려준다 — 조회 서비스가 이미 `MediaPurpose` 로 그 갈림을 다루고 있다.
...
상한(24시간)에 닿거나 시도 횟수를 다 쓰면 **승격하고 `APPROVED` 로 확정한다.**
