...
- **피드 정렬은 최신순(`id DESC`)과 좋아요순 둘이다.** 기본은 최신순 — 파라미터를 안 보내던 클라이언트의 화면이 조용히 달라지면 안 된다.
  - **좋아요순은 상위 N개만 주고 페이징하지 않는다.** 정렬 키가 스크롤 도중 바뀌어 페이지를 이어 붙일 수 없다. 좋아요가 하나 눌리면 그 인증이 위로 올라가, 다음 페이지에서 이미 본 항목이 다시 나오거나 밀려 내려간 항목을 건너뛴다. 커서를 `verified_at` 이 아니라 `id` 로 잡은 것과 같은 근거이고, **좋아요 수는 `verified_at` 보다 훨씬 자주 바뀐다.**
  - 그래도 **응답 형태는 최신순과 같게** 둔다(`nextCursor = null`, `hasNext = false`). 클라이언트가 커서 규약을 그대로 따르면 한 페이지에서 멈추므로, **나중에 페이징을 붙여도 계약이 깨지지 않는다.**
