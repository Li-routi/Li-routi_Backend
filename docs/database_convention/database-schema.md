...
> **둥지는 아직 정하지 못했다.** 한 물건인데 캐릭터를 사이에 두고 **두 번 그려진다** — 지금 규칙인 "한 아이템 = 한 자리 = 한 장"을 깨는 첫 사례다. [아바타 아이템] 절에 *"여러 슬롯을 덮는 세트가 생기면 그때 연결 표를 더한다"* 고 적어 둔 그 경우다. 둥지가 **사는 것인지, 캐릭터에 딸린 것인지, 모두 공통인지**에 따라 표가 갈리므로 아래 [아직 정하지 못한 것]에 남긴다.
...
- **둥지가 무엇인가.** 사는 아이템이면 `avatar_item` 한 행에 이미지 두 장을 매다는 연결 표(`avatar_item_layer`)가 필요하고, 캐릭터에 딸린 것이면 `character` 에 컬럼 둘이면 되고, 전 캐릭터 공통 한 벌이면 표가 아예 필요 없다. **응답 계약은 셋 다 같다** — 서버가 정렬된 레이어 배열을 내리는 것은 변하지 않으므로, 이것 때문에 프론트 합의가 늦어질 이유는 없다
