...
> **심사 보류 컬럼 셋(`review_status` · `review_attempts` · `pending_since`)은 도입되어 표에 들어가 있다.** 각 값의 뜻과 도입 근거는 [심사가 장애로 답을 못 주면 보류한다](#심사가-장애로-답을-못-주면-보류한다--설계안-합의-필요)에 있다. 그 절은 결정 배경을 남긴 기록이지 미결 항목이 아니다.
