...
**넷은 서로 겹치지 않는다.** 한 아이템이 한 슬롯만 차지하므로, 무엇을 입으면 무엇이 벗겨지는지를 따로 정의할 필요가 없다 — 같은 슬롯의 이전 아이템이 교체될 뿐이다.
...
**슬롯이 늘어도 스키마는 바뀌지 않는다.** 배경처럼 새 자리가 생기면 `slot` 값 하나와 아이템 시드만 더하면 된다. 슬롯을 컬럼이 아니라 행으로 둔 이유다.

#### 화면의 "전체" 탭은 슬롯이 아니다

탭은 **전체 · 머리 · 얼굴 · 몸 · 손** 다섯이지만 **슬롯은 넷이다.** `전체` 는 분류가 아니라 **필터 없음**을 뜻한다 — 목록 요청에서 `slot` 을 생략하면 전부 내려간다.
...
미보유 아이템에는 **구매 버튼이 따로 있다.** 요청은 아이템 id 하나면 된다 — 가격과 결제 재화는 `avatar_item` 이 갖고 있으므로 **클라이언트가 보낸 값을 믿지 않는다.**

```text
POST /api/shop/items/{itemId}/purchase

⓪ 판매 중인지  active = 1 이 아니면 거절
① 보유 행 생성  구매 시점 단가·재화를 스냅샷
② 잔액 차감    아이템의 currency · price 로
③ 착용 교체    그 아이템의 슬롯만
```

**①②③ 은 한 트랜잭션이다.** 나뉘면 재화만 빠지고 아이템이 안 생기거나, 샀는데 안 입혀진 상태가 남는다.
...
**⓪ 이 없으면 판매를 내려도 계속 팔린다.** `active` 는 목록에서 빼는 값이 아니라 **파는가 마는가**이므로, 아이템 id 를 아는 클라이언트는 목록을 거치지 않고 바로 구매를 부를 수 있다. 목록에서 감추는 것과 구매를 막는 것은 다르다.

**③ 은 그 아이템의 슬롯만 건드린다.** 다른 슬롯의 착용은 그대로다 — 손에 든 것을 샀다고 모자가 벗겨지면 안 된다. 같은 슬롯에 이미 입은 것이 있으면 새로 산 것으로 **교체**된다(행이 하나이므로 자연히 그렇게 된다).
...
> **아직 구현되지 않았다.** 결제는 **포트원(PortOne)** 으로 간다. 아래 [먼저 정할 것] 이
> 정해지기 전에는 환불 처리를 만들 수 없다.
...
- **받는 재화는 무상으로 들어간다.** `TOPAZ` 는 무료 재화라 유상 잔액이 있을 수 없다 — 위 [무료 재화에 유상 잔액을 만들 수 없다] 를 볼 것. 이는 **"교환은 사용이다"** 라는 뜻이기도 하다: `GEM` 을 낸 시점에 그 재화를 쓴 것이므로 환불 책임이 거기서 끝난다. **약관에 명시해야 한다.**
...
| merchant_uid | VARCHAR(64) | N | **우리가 만드는 주문번호.** `UNIQUE` |
| payment_id | VARCHAR(64) | Y | **포트원이 만드는 결제 식별자.** `UNIQUE`. 결제 전에는 비어 있다 |
| status | VARCHAR(20) | N | `READY` · `PAID` · `FAILED` · `CANCELLED` |
| expected_amount | INT | N | **결제 시작 시점에 서버가 기록한 금액** |
| paid_amount | INT | Y | 실제 결제된 금액 |
| requested_at | DATETIME(6) | N | 결제 시작 시각 |
| paid_at / failed_at / cancelled_at | DATETIME(6) | Y | 상태 전이 시각 |
| fail_reason | VARCHAR(255) | Y | 실패·취소 사유 |
| created_at / updated_at | DATETIME(6) | N / N | 생성·수정 시각 |
...
① 결제 시작   POST /api/shop/charges
              서버가 charge_payment 를 READY 로 만들고 merchant_uid 와
              expected_amount 를 기록한 뒤 클라이언트에 내려준다

② 결제창      클라이언트가 포트원 SDK 로 결제한다

③ 검증·지급   POST /api/shop/charges/{merchantUid}/complete
              서버가 포트원 결제 조회 API 로 실제 결제를 확인하고
              expected_amount 와 대조한 뒤 지갑에 넣는다
```
...
- 포트원이 말하는 상태가 **결제 완료**인가
- 금액이 `expected_amount` 와 **정확히 같은가**
- 그 `payment_id` 로 **이미 지급하지 않았는가** (유니크가 최종 방어선)

지급은 지갑을 그대로 쓴다 — `reward_amount` 를 유상으로, `bonus_amount` 를 무상으로 넣는다. 멱등 키는 `charge:{charge_payment.id}` 다.

### 웹훅이 없으면 결제만 되고 지급이 안 되는 경우가 남는다

③ 은 **클라이언트가 불러 준다.** 결제 직후 앱이 죽거나 네트워크가 끊기면 **돈은 나갔는데 재화가 없는** 상태가 된다.

**포트원 웹훅을 받아 같은 처리를 한다.** 둘 중 먼저 도착한 쪽이 지급하고, 나중 것은 멱등으로 조용히 통과한다 — 지갑의 멱등 키가 그 역할을 한다.

### 🔓 먼저 정할 것

#### 1. 환불로 잔액이 음수가 되면 어떻게 하는가 — **법무·기획**

충전한 재화를 **이미 아바타 아이템에 써 버린 뒤** 환불을 요구하면, 회수 결과가 음수가 된다.

| | 방식 | 대가 |
| --- | --- | --- |
| 가 | **환불을 거절한다** | 미사용분만 환불. 전자상거래법상 미사용 디지털 재화가 청약철회 대상이라는 점과 어긋나지 않는다 |
| 나 | **음수를 허용한다** | 환불은 되지만 사용자가 빚을 진다. 갚기 전에는 아무것도 못 산다 |
| 다 | **산 아이템을 회수한다** | 계산이 맞는다. 아이템 보유를 뺏는 것이라 사용자 반발이 크다 |

**「가」를 제안한다.** 다만 **"미사용" 의 정의**(충전분만 보는가, 지금 잔액을 보는가)가 정해져야 한다.

#### 2. 미성년자 결제 취소 — **법무**

법정대리인 동의가 없으면 취소 가능하다. 이 경우도 위 1 과 같은 상황이 되지만 **거절할 수 없다.**

#### 3. 어느 PG 로 테스트할 것인가

포트원은 **계약 전에도 테스트 채널로 붙을 수 있다.** 다만 채널을 만들 때 PG 사를 고르고, 결제창 방식이 조금씩 달라 **프론트와 맞춰야 한다.**

#### 4. 포트원 자격증명

가맹점 식별코드와 API 키가 필요하다. **환경변수로 주입하고 커밋하지 않는다** — 진짜 비밀값이므로 기본값 없이 선언해 미주입 시 부팅이 실패하게 한다.

> ⚠️ **실제 앱스토어·플레이스토어 배포 시에는 외부 PG 가 막힌다.** Apple·Google 은 앱 안에서 소비되는 디지털 재화를 자사 인앱결제로만 팔게 한다. **웹 배포이거나 심사를 받지 않는 경우에만 이 설계가 성립한다.**
