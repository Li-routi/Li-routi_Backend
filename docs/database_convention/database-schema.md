...
- 신고 누적으로 전체 회원에게 숨기는 처리는 **아직 없다.** 따라서 `hidden_at` 같은 컬럼도 두지 않는다. 그래서 신고하지 않은 회원에게는 부적절한 사진이 계속 노출된다. **임계값 기반 자동 숨김을 #60에서 다루기로 했으므로, 그때 이 서술과 스키마를 함께 갱신한다.**
...
파일명에 클라이언트가 보낸 값을 쓰지 않는다(경로 조작 방지). UUID라 다른 회원의 미디어 경로를 유추할 수 없고, 공개 prefix에 `s3:ListBucket`을 주지 않으므로 열거도 불가능하다.
