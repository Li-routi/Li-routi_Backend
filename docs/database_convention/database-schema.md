...
WHERE member_id = :memberId
  AND all_completed = 1
  AND activity_date > CURRENT_DATE - INTERVAL 15 DAY
```
...
- **그날 예정된 개인 루틴이 0개인 날을 어떻게 볼 것인가.** 개인 루틴은 요일 패턴이라 "주 5일" 로 잡으면 주말에 예정이 0개다. 실패로 보면 **주말을 비운 사람은 둥지 레벨 2 에 영원히 닿지 못하고**(15일이든 30일이든 6일을 못 넘긴다), 건너뛰기로 보면 요일을 하나만 잡아 두고 15주를 채울 수 있다. 운영에는 지금 활성 루틴을 가진 회원이 아홉이고 **전원이 7요일을 덮고 있어** 아직 드러나지 않은 문제다(표본이 작다)
