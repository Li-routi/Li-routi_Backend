...
업적 조건 달성  →  member_achievement.status = ACHIEVED  →  사용자가 "받기"
                              ...
                              AchievementClaimService → CharacterUnlockService.unlockByAchievementClaim
                                                              ↓
                                         ACHIEVEMENT_CLAIMED 행을 찾아 member_character 삽입
```

| 캐릭터 | 업적 코드 | 업적 |
| --- | --- | --- |
| 민트 | `ACH-EG-001` | 첫 루틴의 새싹 |
| 동글 | `ACH-EG-002` | 배움이 차곡차곡 |
| 노아 | `ACH-EG-003` | 일찍 일어난 새 |
| 코코 | `ACH-EG-004` | 건강한 땀방울 |
| 유키 | `ACH-EG-005` | 마음에 쉼표 |
| 까루 | `ACH-EG-007` | 알림 보고 왔어요 |
| 다미 | `ACH-EG-008` | 정리하면 다미 |
| 모리 | `ACH-EG-009` | 자정의 방문자 |
| 삐아 | `ACH-EG-010` | 좋아요 요정 |
| 호롱 | `ACH-EG-011` | 딱 1분 남았어! |
| 솔라 | `ACH-EG-012` | 100일의 태양 |
| 파도 | `ACH-EG-013` | 파도의 도전 |

**`ACH-EG-006`(취미의 물결) 은 이 표에 없다.** 파도의 옛 업적인데 `ACH-EG-013` 으로 대체됐다 — 기획 문서의 "캐릭터 획득 조건 최종 정리" 표와 "세부조건(연속형 규칙)" 이 서로 다른 조건을 적고 있었고, 연속형이 최신이라 그쪽으로 갔다. 대체된 쪽은 `active = 0` 으로 내렸다(`V20260819223000`).

> ⚠️ `achievement_reward_item` 을 캐릭터↔업적 매핑의 근거로 삼지 말 것. **코드가 읽지 않는 표라 갱신이 밀린다** — 실제로 파도가 옮겨간 뒤에도 옛 매핑(`ACH-EG-006` → 파도의 알)이 남아 있었고, 그것을 최신으로 착각해 시드를 잘못 쓴 적이 있다. 근거는 `character_unlock_condition` 시드다.

**난이도는 `achievement.condition_key` / `target_count` 가 정한다.** 여기서 다시 정하지 않는다 — 두 곳에 수치를 두면 어느 쪽이 진짜인지 알 수 없게 된다.

##### 왜 직접 조건을 걷어냈나

처음에는 캐릭터마다 직접 조건(`ACTIVE_DAYS` · `CATEGORY_DAYS` · `TIME_WINDOW_DAYS` …)을 걸었다. 그러자 **한 캐릭터를 여는 길이 둘이 되었다** — 업적의 "받기" 와, 조건 판정. 그리고 조건 쪽이 늘 먼저 열었다.

실제로 그렇게 됐다. 민트의 직접 조건이 `ACTIVE_DAYS = 1` 이라 **인증을 한 번만 해도 열렸고**, 그래서 회원들이 업적을 받기도 전에 민트를 갖고 있었다. 화면에서는 업적 목록의 "받기" 가 활성인데 상점의 캐릭터는 이미 해금된 상태로 보였다 — 눌러도 아무 일이 일어나지 않는다.

두 벌의 조건은 애초에 같은 것을 두 번 적은 것이었고, **기획의 단일 진실 공급원은 업적 쪽**이다. 그래서 직접 조건을 전부 걷어내고 업적에 붙였다.

**그렇게 얻은 보유는 회수했다** — `V20260819210000__revoke_characters_unlocked_without_claim.sql`. 다만 **실제로 "받기" 를 누른 사람의 것은 남긴다**(`member_achievement.status = 'CLAIMED'`). 받은 사람까지 회수하면 이미 얻은 보상을 빼앗는 것이 된다. 실측으로 개발 3건 회수·1건 유지, 운영 3건 회수였다.

> 그 마이그레이션은 캐릭터↔업적 매핑을 **파일 안에 직접 적는다.** `character_unlock_condition` 을 조인하고 싶지만 그 표는 `R__` 시드가 채우고, **반복 마이그레이션은 `V__` 가 전부 끝난 뒤에 돈다** — `V__` 시점에는 아직 옛 조건이 들어 있어 조인이 성립하지 않는다.

#### 판정기 여덟

> **아래 여덟은 지금 캐릭터 해금에 쓰이지 않는다.** 코드에는 남아 있지만 시드에 걸린 행이 없다. 지우지 않은 것은 조건을 다시 데이터로 걸고 싶어질 때를 위해서다. 이 절과 이어지는 설계 근거도 그때를 위해 남겨 둔다.
...
> 위 [왜 직접 조건을 걷어냈나] 의 회수 마이그레이션은 이 규칙의 예외가 아니다. 잘못된 경로로 생긴 행을 지우는 일회성 정리이지, 조건 미달을 감지해 되돌리는 상시 동작이 아니다.
...
## 마이페이지 — 건의 테이블 — 설계안 (합의 필요)

사용자가 **건의를 보내고, 자기가 보낸 것을 다시 본다.** 마이페이지 화면에 붙는다.

### 왜 `mypage` 도메인인가

건의만 놓고 보면 `suggestion` 이 더 정확한 이름이다. 그럼에도 `mypage` 아래에 두는 것은 **마이페이지에 붙을 것이 건의 하나로 끝나지 않기 때문**이다 — 공지사항·약관·설정 같은 것이 같은 화면에 모인다. 그때마다 도메인을 하나씩 만들면 한 화면이 도메인 예닐곱 개로 흩어진다.

> 건의가 커져 독립할 만해지면 그때 옮긴다. 표 이름은 `suggestion` 이므로 도메인만 바뀐다.

### `suggestion_category`

건의를 등록할 때 고르는 분류다. **앱이 제공하는 마스터 데이터**이고 `R__` 시드가 단일 진실 공급원이다.

| 컬럼 | 타입 | NULL | 설명 |
| --- | --- | --- | --- |
| id | BIGINT | N | 기본 키. 시드가 직접 지정한다 |
| code | VARCHAR(30) | N | 논리 키 |
| name | VARCHAR(30) | N | 화면에 쓸 이름 |
| display_order | INT | N | 노출 순서 |
| active | TINYINT(1) | N | 사용 여부, 기본값 `1` |
| created_at / updated_at | DATETIME(6) | N / N | 생성·수정 시각 |

유니크: `uk_suggestion_category_code` (`code`)

**enum 이 아니라 표로 둔다.** 분류를 하나 더하는 일이 **배포 없이** 되어야 하고, 나중에 백오피스가 이 목록을 관리한다. 챌린지 카테고리가 실제 MySQL `ENUM` 이라 값 하나 더하는 데 `ALTER TABLE` 이 필요했던 것과 반대 선택이다.

**내린 분류도 지우지 않는다.** `active = 0` 으로 목록에서만 빼면, 그 분류로 이미 보낸 건의가 참조를 잃지 않는다. 아바타 아이템의 판매 종료와 같은 방식이다.

#### 시드는 "어디에 대한 건의인가" 로 나눈다

| id | code | name | 상태 |
| --- | --- | --- | --- |
| 5 | `MAIN` | 메인 | 사용 |
| 6 | `GROUP` | 그룹 | 사용 |
| 7 | `GROUP_CHAT` | 그룹채팅 | 사용 |
| 8 | `CHALLENGE` | 챌린지 | 사용 |
| 4 | `ETC` | 기타 | 사용 |
| 1 | `BUG` | 버그 신고 | 내림(`active = 0`) |
| 2 | `FEATURE` | 기능 제안 | 내림(`active = 0`) |
| 3 | `INCONVENIENCE` | 불편 사항 | 내림(`active = 0`) |

**처음에는 "어떤 종류의 건의인가"(버그·기능 제안·불편)로 나눴다.** 분류 개수가 안정적이고 어디인지는 본문이 말해 준다는 이유였다. 실제로는 **사용자가 자기 불편이 버그인지 기능 제안인지 가르기 어려워한다.** 화면 단위는 그 판단이 필요 없고, 접수된 건의가 어느 화면 것인지도 분류만 보고 갈린다. 화면이 늘면 분류도 늘어나는 것은 감수한다.

**`ETC` 는 id 를 유지했다.** 뜻이 그대로라 이미 이 분류로 보낸 건의가 계속 유효하다. 나머지 셋은 **지우지 않고 `active = 0` 으로 내렸다** — 지우면 그 분류로 보낸 건의가 참조를 잃고, `R__` 는 upsert 라 시드에서 줄을 지워도 DB 에서는 사라지지 않는다.

**이 판단은 되돌리기 싸다.** `R__` 시드라 배포만으로 바뀐다. 다만 **`id` 는 재사용하지 않는다.** 이미 그 분류로 보낸 건의가 다른 뜻을 가리키게 된다 — 새 분류에 5~8 을 쓴 것이 그래서다.

#### `active` 가 세 경로에서 다르게 동작한다

이 값을 "목록에서 뺀다" 로만 적어 두면 나머지 둘이 정해지지 않는다.

| 경로 | 비활성 분류를 | 왜 |
| --- | --- | --- |
| 분류 목록 조회 | **뺀다** (`WHERE active = 1`) | 더 이상 받지 않는 분류를 고르게 두지 않는다 |
| 건의 등록 | **거절한다** | 목록에 없어도 id 를 아는 클라이언트는 그대로 보낼 수 있다. 아이템의 판매 종료 확인과 같은 자리다 |
| 내 건의 목록 | **그대로 보여준다** | 분류가 내려갔다고 내가 보낸 건의의 분류명이 사라지면 안 된다 |

**목록 조회는 `active` 로 거르지 않는다** — 건의가 이미 그 분류를 가리키고 있으므로 조인해서 `code` 와 `name` 을 그대로 싣는다.

**정렬을 계약으로 못박는다.** DB 기본 순서는 보장되지 않는다.

```sql
SELECT * FROM suggestion_category
WHERE active = 1
ORDER BY display_order ASC, id ASC
```

`id ASC` 를 뒤에 두는 것은 `display_order` 가 같은 둘이 생겼을 때 순서가 흔들리지 않게 하려는 것이다.
...
| --- | --- | --- | --- |
| id | BIGINT | N | 기본 키 |
| member_id | BIGINT | N | 보낸 사람, `member.id` FK |
| suggestion_category_id | BIGINT | N | `suggestion_category.id` FK |
| title | VARCHAR(100) | N | 제목. 목록 표시와 검색이 쓴다 |
| content | VARCHAR(2000) | N | 본문 |
| status | VARCHAR(20) | N | 처리 상태. `DEFAULT 'RECEIVED'` |
| created_at / updated_at | DATETIME(6) | N / N | 생성·수정 시각 |

인덱스: `idx_suggestion_member_created` (`member_id`, `id` DESC)

#### 제목을 뒤늦게 더했다

**처음에는 두지 않았다.** 분류가 제목 노릇을 하고, 칸이 하나 늘면 건의 자체가 줄어든다고 봤다. 실제로는 **목록에서 무슨 건의인지 알아볼 방법이 분류 이름뿐**이라 같은 분류의 건의가 전부 같아 보였고, 본문 앞부분을 잘라 보여주면 "안녕하세요" 로 시작하는 것들이 구별되지 않았다. 검색을 붙일 자리도 없었다.

**앞뒤 공백을 떼고 저장한다.** 검색이 제목을 그대로 맞춰 보므로, 눈에 안 보이는 공백이 붙어 있으면 사용자가 화면에 보이는 대로 검색해도 안 걸린다. 공백만 있는 제목은 빈 제목으로 보고 거절한다(`SUGGESTION400_3`).

**이미 쌓인 건의의 제목은 본문 앞 100자로 채웠다**(`V20260820103000`). 지어낸 값("제목 없음")보다 낫다 — 운영이 목록에서 무엇에 대한 건의인지 알아볼 수 있어야 하는데 그 정보가 있는 곳은 본문뿐이다. 줄바꿈은 공백으로 바꿨다. 그대로 두면 한 줄짜리 제목 안에 개행이 들어가 목록이 깨진다.

#### 검색은 제목과 분류로 거른다

```text
GET /api/members/me/suggestions?keyword=알림&categoryId=5
```

| 조건 | 무엇을 거르나 |
| --- | --- |
| `keyword` | **제목** 부분 일치 |
| `categoryId` | 그 **분류**의 것만 |

**둘 다 주면 둘 다 만족하는 것만** 나온다(AND). 둘 다 비우면 전체다.

**본문까지 뒤지지 않는다.** 긴 글 안의 아무 단어나 걸려 목록이 뭉개진다 — 제목은 사용자가 그 건의를 부르려고 지은 이름이므로, 찾을 때 기대하는 것도 그쪽이다.

**`categoryId` 는 내려간 분류도 받는다.** 그 분류로 이미 보낸 건의를 찾을 수 있어야 한다 — 고를 수 없게 하는 것과 찾을 수 없게 하는 것은 다르다. 목록 조회가 `active` 로 거르지 않는 것과 같은 이유다.

**다만 없는 분류 id 는 거절한다**(`SUGGESTION404_1`). 조용히 빈 목록을 주면 "이 분류에는 건의가 없다" 로 읽혀, 앱이 잘못된 id 를 보내고 있다는 것을 알아챌 기회가 사라진다.

**`%` 와 `_` 를 이스케이프한다.** 그대로 넘기면 사용자가 친 `%` 가 와일드카드가 되어 전부 걸리고, `_` 는 아무 글자 하나와 맞는다. 그러면 "찾은 것" 과 "안 거른 것" 을 구별할 수 없다. 이스케이프 문자로 `!` 를 쓰므로 **`!` 자체를 먼저 이스케이프한다** — 순서를 바꾸면 앞서 넣은 이스케이프 문자까지 다시 이스케이프된다.

**빈 검색어는 검색하지 않은 것으로 접는다.** 그대로 패턴으로 만들면 `'%%'` 가 되어 전부 걸리는데, 그것은 "검색해서 다 나왔다" 와 "검색어가 없어서 다 나왔다" 를 구별할 수 없게 만든다.

**대소문자를 구별하지 않는다.** 컬럼 콜레이션이 `utf8mb4_unicode_ci` 라 비교가 이미 그렇다 — 애플리케이션에서 `lower()` 를 부르면 같은 일을 두 번 한다.

> **검색용 인덱스를 두지 않았다.** `title LIKE '%...%'` 는 앞이 열려 있어 B-Tree 인덱스가 쓰이지 않는다. 만들면 쓰이지도 않으면서 쓰기 비용만 늘고, "인덱스가 있으니 빠르겠지" 라는 오해를 남긴다. 그래도 괜찮은 것은 조회가 언제나 `member_id` 로 먼저 좁혀지기 때문이다 — `idx_suggestion_member_created` 가 자기 건의만 남기고 `LIKE` 는 그 안에서만 돈다. 한 사람 앞에 수천 건이 쌓이는 데이터가 되면 그때 `FULLTEXT` 를 검토한다.

**걸러도 커서 방식은 그대로다.** 같은 조건(`keyword`·`categoryId`)을 유지한 채 `nextCursor` 를 넘겨야 다음 쪽이 이어진다. 조건을 바꾸면 커서를 버리고 처음부터 받는다.

#### 본문은 `TEXT` 가 아니라 `VARCHAR(2000)` 이다

상한이 있어야 요청 DTO 가 그것을 검증할 수 있고, 비정상적으로 큰 본문이 들어오는 것을 **DB 앞에서** 막는다. 2000자를 넘겨야 할 건의는 사실상 없다.

#### 상태는 지금 움직이지 않는다

| 값 | 뜻 |
| --- | --- |
| `RECEIVED` | 접수됨. **지금은 모든 건의가 여기 머문다** |
| `IN_PROGRESS` | 처리 중 |
| `DONE` | 처리 완료 |

**상태를 바꾸는 경로가 아직 없다.** 백오피스가 생기면 그때 관리자 API 가 붙는다. 그럼에도 컬럼을 지금 두는 이유는, 나중에 더하면 **이미 쌓인 건의 전부에 기본값을 소급 부여**해야 하고 그 시점에는 무엇이 처리됐는지 알 방법이 없기 때문이다.

> 화면에는 전부 "접수" 로 보인다. 그것이 사실이므로 감추지 않는다.

**허용값은 자바 enum 하나가 정한다.** 컬럼은 `VARCHAR(20)` 에 `DEFAULT 'RECEIVED'` 이고, `@Enumerated(EnumType.STRING)` 으로 그 셋만 오간다. 이 저장소가 상태·구분 값을 다루는 방식과 같다 — 챌린지 카테고리가 실제 MySQL `ENUM` 이었다가 값 하나 더하는 데 `ALTER TABLE` 이 필요했던 전례가 있어, 상태에는 DB `ENUM` 을 쓰지 않는다.

> 그래서 **DB 만으로는 잘못된 문자열을 막지 못한다.** 쓰는 경로가 애플리케이션 하나뿐이라는 사실에 기대고, 나중에 백오피스가 상태를 바꿀 때도 같은 enum 을 지난다.

**답변 본문은 이번 범위에 없다.** 상태만으로 "봤다" 를 알릴 수 있고, 답변을 두려면 답변 표와 관리자 화면이 함께 필요하다. 필요해지면 `suggestion_answer` 를 더한다 — 한 건의에 답이 여럿 달릴 수 있으므로 컬럼이 아니라 표다.
...
GET  /api/members/me/suggestions              내 건의 목록 (?keyword=제목 &categoryId=분류)
```
...
**커서 방식은 챌린지·채팅과 같다.** `id DESC` 가 곧 최신순이라 정렬 키를 따로 두지 않는다 — `created_at` 은 같은 초에 둘이 들어오면 순서가 흔들린다.

`size + 1` 을 읽어 **한 건이 남으면 `hasNext = true`**, 그 여분은 응답에서 버리고 마지막 항목의 `id` 를 `nextCursor` 로 내린다. `hasNext` 가 `false` 면 `nextCursor` 는 `null` 이다.

#### `size` 에 상한을 둔다

**상한이 없으면 회원이 자기 건의 전부를 한 번에 끌어갈 수 있다.** 큰 값을 넣는 것만으로 그렇게 되므로 요청 DTO 에서 막는다.

| | 값 |
| --- | --- |
| 기본 | 20 |
| 범위 | `1 ≤ size ≤ 50` |
| 벗어나면 | `400` — 조용히 잘라 맞추지 않는다 |

조용히 상한으로 깎으면 클라이언트는 자기가 요청한 수와 받은 수가 다른 이유를 알 수 없다.

#### `memberId` 는 인증 주체에서만 나온다

**경로·쿼리·본문에서 받지 않는다.** `@AuthenticationPrincipal CustomUserDetails` 의 `memberId` 를 쓴다 — 요청이 주는 값을 믿으면 `WHERE member_id = :memberId` 가 격리를 전혀 보장하지 못하고, 남의 id 를 넣는 것만으로 남의 건의를 읽는다.

경로가 `/api/members/me/...` 인 것도 같은 이유다. `/api/members/{memberId}/suggestions` 로 두면 그 값을 검증하는 코드가 매번 필요해진다.

> **테스트로 지킨다.** "다른 회원의 건의는 내 목록에 섞이지 않는다" 를 통합 테스트에 넣는다 — 이 규칙은 코드를 읽어서는 깨진 것을 알아채기 어렵다.

**남의 건의는 애초에 보이지 않는다.** 마이페이지의 "내 건의 내역" 이고, 공개 목록으로 만들면 신고·숨김·작성자 표시 정책이 통째로 따라온다.

#### 스팸을 막는 장치는 아직 없다

한 사람이 연달아 보내는 것을 막지 않는다. 지금 규모에서는 과한 방어이고, 필요해지면 기존 요청 빈도 제한(`RateLimitProperties`)에 이 경로를 더하는 것이 자연스럽다. **막지 않기로 한 것이 아니라 아직 안 한 것**이므로 적어 둔다.
