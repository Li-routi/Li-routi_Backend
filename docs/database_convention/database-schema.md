...
| 코드값 | 시안 표기 | 얻는 법 |
| --- | --- | --- |
| `GEM` | 파란 보석 (충전 팝업에서는 "코인") | 현금 결제 · 보너스 |
| `TOPAZ` | 주황 보석 ("토파즈") | `GEM` 으로 교환 |
...
> **`TOPAZ` 의 사용처는 아직 정해지지 않았다.** 시안에서 아바타 아이템 가격은 전부 `GEM` 이고 `TOPAZ` 를 쓰는 화면이 없다. 챌린지 `reward`("무료 재화")가 어느 쪽인지도 미정이다. **이 표는 두 재화를 대칭으로 다루므로 어느 쪽으로 결정되든 스키마는 바뀌지 않는다.**
...
| idempotency_key | VARCHAR(150) | N | **멱등 키.** `(member_id, idempotency_key)` 유니크 |
| reference_type | VARCHAR(30) | Y | 근거가 된 대상의 종류(예: 인증·아이템·충전) |
| reference_id | BIGINT | Y | 그 대상의 id |
| created_at / updated_at | DATETIME(6) | N / N | 생성·수정 시각 |

- `UNIQUE(member_id, idempotency_key)` — **같은 요청이 두 번 들어와도 두 번 반영되지 않는다.** 네트워크가 끊겨 클라이언트가 재시도하는 것은 정상 동작이고, 그때 두 번 차감되거나 두 번 지급되면 그대로 돈 문제가 된다.
