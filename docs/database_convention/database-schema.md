...
> **1·2·3단계가 모두 구현됐다.** 인증 사진은 조회 API가 내려주는 **서명된 한시적 주소**로
> 볼 수 있고(1단계), 그룹·개인 루틴 인증이 동작하며(3단계), 챌린지 인증도 `verification`
> 도메인으로 옮겨와(2단계) 세 대상의 인증이 한 패키지에 모였다. 남은 것은 **key 범위 식별자**다.
>
