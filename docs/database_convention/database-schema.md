...
> ⚠️ **실제 결제를 받기 시작하면 이 결정을 반드시 다시 본다.** 전자상거래법상 **미사용 디지털 재화의 청약철회는 사업자가 거부할 수 없다.** 그때 정해야 할 것은 그대로 남아 있다 — **이미 아이템에 써 버린 재화를 환불하면 잔액이 음수가 되는데**(거절 / 음수 허용 / 아이템 회수), 그리고 **미성년자 결제 취소는 거절할 수 없다.**
...
- **플레이스토어 배포 예정이 없다.** Apple·Google 은 앱에서 소비되는 디지털 재화를 자사 인앱결제로만 팔게 하므로 외부 PG 는 심사에서 막히는데, 심사를 받지 않으면 해당하지 않는다. **배포를 결정하는 시점에 이 설계 전체를 다시 봐야 한다** — 그때는 결제 모델이 통째로 바뀐다.
