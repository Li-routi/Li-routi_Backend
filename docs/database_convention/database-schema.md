...
```text
GET /api/members/me/suggestions?keyword=알림
```
...
**검색해도 커서 방식은 그대로다.** 같은 `keyword` 를 유지한 채 `nextCursor` 를 넘겨야 다음 쪽이 이어진다. 검색어를 바꾸면 커서를 버리고 처음부터 받는다.
...
GET  /api/members/me/suggestions              내 건의 목록 (?keyword= 로 제목 검색)
```
