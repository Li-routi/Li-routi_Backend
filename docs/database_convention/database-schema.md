...
| 코드값 | 시안 표기 | 성격 | 얻는 법 |
| --- | --- | --- | --- |
| `GEM` | 파란 보석 | **무료 재화** | 챌린지 인증 리워드 · `TOPAZ` 로 구매 |
| `TOPAZ` | 주황 보석 ("토파즈", 충전 팝업에서는 "코인") | **유료 재화** | 현금 결제 · 충전 보너스 |

**둘은 서로를 대신하지 못한다.** `GEM` 으로만 살 수 있는 아이템이 있고 `TOPAZ` 로만 살 수 있는 아이템이 따로 있다. 한쪽이 모자란다고 다른 쪽으로 결제되지 않는다.

**교환은 `TOPAZ` → `GEM` 한 방향뿐이다.** 상점에서 `TOPAZ` 를 주고 `GEM` 을 산다. 반대 방향은 없다 — 유료 재화를 무료 활동으로 얻을 수 있게 되면 결제가 무의미해진다.
...
**`GEM` 은 전부 무상이다.** 챌린지로 벌든 `TOPAZ` 로 사든 `GEM` 은 무료 재화이므로 `GEM.paid_balance` 는 쓰지 않는다(항상 `0`).

> **`TOPAZ` 를 `GEM` 으로 바꾼 것은 "사용"이다.** 결제한 재화를 이미 쓴 것이므로 그 뒤에 남은 `GEM` 은 청약철회 대상이 아니다. 교환 시점에 환불 책임이 끝난다는 뜻이라, **약관에 명시해야 한다**(결제 이슈에서 다룬다).

**유상·무상이 실제로 갈리는 곳은 `TOPAZ` 뿐이다** — 현금으로 산 몫(`paid`)과 충전 보너스(`free`)다.

> ⚠️ **`GEM` 에 유상 잔액을 만들지 말 것.** 리워드 회수는 `GEM` 에서 일어나는데, `GEM.paid_balance` 가 생기는 순간 **회수가 환불 대상 재화를 깎기 시작한다.** 지금은 그 값을 만드는 경로가 없어서 안전할 뿐이고, 이 안전은 코드가 아니라 **관행에 얹혀 있다.**
...
> **`GEM` 에서는 이 순서가 눈에 띄지 않는다.** 유상 잔액이 없으므로 항상 무상에서만 빠진다. 실제로 갈리는 것은 `TOPAZ` 다.
...
여기서 말하는 "잔액"은 **그 재화의 잔액**이고, 리워드는 `GEM` 이므로 `GEM` 잔액이다. `GEM` 에는 유상 잔액이 없으므로 이 값은 곧 무상 잔액이다 — 위 [무료·유료는 재화 종류이고…] 참고.
