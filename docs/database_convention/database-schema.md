...
> ⚠️ **`TOPAZ` 에 유상 잔액을 만들지 말 것.** 리워드 회수는 `TOPAZ` 에서 일어나는데, `TOPAZ.paid_balance` 가 생기는 순간 **회수가 환불 대상 재화를 깎기 시작한다.** 지금은 그 값을 만드는 경로가 없어서 안전할 뿐이고, 이 안전은 코드가 아니라 **관행에 얹혀 있다.**
...
> **아직 구현되지 않았다.** 이 절은 합의를 받기 위한 설계안이고, 아래 [먼저 정할 것] 두 가지가
> 정해지기 전에는 착용 API를 만들 수 없다.
...
**아직 정해지지 않았다.** 아래 [먼저 정할 것] 2번을 볼 것.

`slot` 은 코드값만 갖고 화면 문구는 클라이언트가 붙인다 — 재화 이름·스트릭 문구와 같은 기준이다.

### 🔓 먼저 정할 것

#### 1. 안 산 아이템을 골랐을 때 무슨 일이 일어나는가

화면에는 **구매 버튼이 없고 CTA 가 "저장하기" 하나뿐**이다. 그런데 격자에는 미보유 아이템도 가격과 함께 뜬다.

| | 방식 | 대가 |
| --- | --- | --- |
| A | 누르면 즉시 구매 확인. 사야 미리보기 | 구현이 단순하고 구매·착용이 깔끔히 갈린다. **실수 구매가 무섭고, 입어 보고 사려는 심리와 반대다** |
| B | 미리보기는 자유, **"저장하기" 에서 일괄 결제** | 화면과 맞는다. 대신 **구매 + 착용이 한 트랜잭션**이 되어 부분 성공을 허용할 수 없다 |
| C | 미리보기는 자유, 저장은 **보유한 것만** | 가장 단순하다. **구매 버튼이 따로 필요해 화면과 안 맞는다** |

**B 를 제안한다.** 다만 B 는 계약이 하나 늘어난다 — **여러 건을 사는 도중 잔액이 모자라면 전부 실패한다.** 어느 것까지 샀는지 애매한 상태를 남기지 않는다.

#### 2. 슬롯 구성과 "세트" 의 점유 범위

탭이 **전체 / 배경 / 모자 / 세트** 다. 세트가 모자를 포함한다면 **모자를 쓴 채로 세트를 고르면 모자가 벗겨져야 한다.**

이것을 데이터로 표현하는 방법이 둘이다.

| | 방식 | 대가 |
| --- | --- | --- |
| 가 | 세트를 **하나의 슬롯**으로 두고, 세트 착용 시 다른 슬롯 행을 지운다 | 표가 단순하다. **"어떤 슬롯을 덮는가" 가 코드에 박힌다** |
| 나 | 아이템이 **점유 슬롯 목록**을 갖는다(`avatar_item_slot` 연결 표) | 세트 구성이 데이터로 바뀐다. 표가 하나 늘고 조회가 복잡해진다 |

**세트 종류가 한 가지 모양(모자+의상 등)으로 고정이면 「가」, 세트마다 덮는 범위가 다르면 「나」다.** 시안만으로는 알 수 없다.

**어느 쪽이든 충돌 정리는 서버가 한다.** 클라이언트에 맡기면 앱 버전마다 규칙이 갈리고, 구버전이 낡은 규칙으로 저장한 상태가 남는다.

#### 3. 기본 제공 아이템이 있는가

가입 시점에 아무것도 안 입은 캐릭터를 보여줄 것인지, 기본 아이템을 입혀 둘 것인지다. 기본 제공이 있으면 `price = 0` 아이템을 시드에 넣고 **가입 시 보유 행을 만들지, 아니면 "가격 0 은 누구나 착용 가능" 으로 볼지**를 정해야 한다.

**후자를 제안한다.** 가입 흐름(다른 도메인)을 건드리지 않아도 되고, 기본 아이템이 늘어도 기존 회원에게 백필할 필요가 없다. 지갑을 가입 시점에 만들지 않기로 한 것과 같은 이유다.
