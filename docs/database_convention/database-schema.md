...
③ 심사   통과하면 challenge-verifications/… 으로 복사하고 대기본을 지운다
         반려하면 대기본만 지운다
④ 공개   공개 URL 은 ③을 지난 뒤에야 존재한다
```

**날짜와 UUID 는 그대로 두고 prefix 만 바꾼다.** 승격 전후의 key 가 한 글자만 다르므로, 정리 배치의 날짜 prefix 훑기가 양쪽에서 똑같이 동작한다.
...
승격은 S3 내부 복사라 전송비가 들지 않지만, **대기본을 지우려면 `s3:DeleteObject` 가 대기 prefix 에도 있어야 한다.** 현재 역할은 그 권한을 `challenge-verifications/` 로만 좁혀 두었다(`deploy/iam-policy.json`). 코드보다 정책이 먼저 넓혀져 있어야 한다 — 아니면 승격은 되는데 대기본이 계속 쌓인다.
...
> **대기 prefix 는 비공개인데 식별자가 없다.** 위 규칙과 어긋나 보이지만 의도한 것이다. 식별자를 두는 이유는 "누가 볼 수 있는가"를 경로로 가르기 위해서인데, 대기본은 **누구도 볼 수 없다.** 승격되면 전체 공개가 되므로 그때도 가를 대상이 없다. 즉 대기는 공개 용도의 한 상태이지 별도의 접근 범위가 아니다.
>
> 클라이언트가 발급을 요청할 때 보내는 용도도 그대로 챌린지 인증이다. **대기 prefix 로 받는다는 것은 서버 내부 사정**이라 API 로 드러내지 않는다.
