...
{
  "layers": [
    { "z": 10, "slot": "NEST_BACK", "imageUrl": "..." },
    { "z": 20, "slot": "CHARACTER", "imageUrl": "..." },
    { "z": 30, "slot": "BODY",      "imageUrl": "..." }
  ]
}
```
...
| **모르는 `slot` 이 와도 그냥 그린다** | 이름으로 분기하지 않는다. 새 레이어를 서버만 고쳐 넣을 수 있는 유일한 방법이다 |
...
| --- | --- |
| 1 | 기본 |
| 2 | **30일 연속, 그날 예정된 개인 루틴을 하나도 빠짐없이 완수** |
...
**30이면 레벨 2, 아니면 1이다.** 중간에 하루가 비면 30일 창에 30개가 들어올 수 없으므로 **끊김을 따로 판정하지 않는다.** 연속일수 컬럼도, 그것을 리셋하는 배치도 두지 않는다 — **매일 자정에 도는 일이 없으므로 배치가 죽어 있는 동안 조용히 틀리는 상태가 생기지 않는다.**
...
**실패를 행으로 남기지 않는다.** 성공한 날에만 표시가 남고, 없는 날은 전부 실패다. 그러면 **루틴이 0개인 날은 아무것도 안 해도 저절로 실패**가 되어 규칙이 하나 줄어든다 — 성공으로 세면 루틴을 전부 지워 두고 30일을 채울 수 있고, 조건이 묻는 "꾸준히 했다" 와 뜻이 반대가 된다.
...
- **그날 예정된 개인 루틴이 0개인 날을 어떻게 볼 것인가.** 개인 루틴은 요일 패턴이라 "주 5일" 로 잡으면 주말에 예정이 0개다. 실패로 보면 **주말을 비운 사람은 둥지 레벨 2 에 영원히 닿지 못하고**, 건너뛰기로 보면 요일을 하나만 잡아 두고 30주를 채울 수 있다. 운영에는 지금 활성 루틴을 가진 회원이 아홉이고 **전원이 7요일을 덮고 있어** 아직 드러나지 않은 문제다(표본이 작다)
