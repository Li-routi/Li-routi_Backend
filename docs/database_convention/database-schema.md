...
인증은 당일 것만 다시 올릴 수 있고, **그것도 `DAILY` 챌린지에서만이다.** 주간·월간은 한 번 올리면 그 구간이 끝나는 성격이라 덮어쓰기를 막는다(409) — 그대로 두면 사진은 수요일 것인데 `verified_date` 는 월요일로 남아 날짜와 내용이 어긋난다. 덮어쓸 때는 행을 지우고 새로 만드는 것이 아니라 **기존 행을 덮어쓴다.** 하루에 한 행이라는 사실이 유지되므로 유니크 제약과 충돌하지 않고, 그 인증을 참조하는 신고 데이터도 그대로 남는다. 어제 이전의 인증은 수정할 수 없으므로 연속 참여일을 소급해 재계산하는 경우는 없다. 집안일의 `canceled_at`에 해당하는 컬럼을 두지 않는 이유다.
