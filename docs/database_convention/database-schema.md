...
> **심사 보류가 도입되면 컬럼이 셋 늘어난다** — `review_status` · `review_attempts` · `pending_since`. 근거와 각 값의 뜻은 [심사가 장애로 답을 못 주면 보류한다](#심사가-장애로-답을-못-주면-보류한다--설계안-합의-필요)에 있다. 아직 합의 전이라 여기 표에는 넣지 않는다.
...
| 오늘 완료 여부 · 완료자 수 | ✅ | 인증은 실제로 했다. 아래 [스트릭은 보류 시점에 올린다] 와 같은 기준이다 |
...
**하루 1회는 보류 건도 그날 인증으로 친다.** 유니크 제약이 자연히 그렇게 만든다. 안 그러면 장애 동안 같은 날 여러 건이 쌓인다.
...
> **구현 완료.** 아래는 합의된 규칙이고 코드가 이대로 따른다.
> 다만 **스트릭 처리는 합의 뒤에 한 번 바뀌었다** — 아래 [오늘 것을 내리면 스트릭에서 뺀다] 참고.
...
| 현재 주기 인증 여부(버튼) | ✅ | **인증한 사실은 남는다.** 그날은 다시 못 한다 |
| 오늘 완료자 수 | ✅ | 수행 기록이지 노출이 아니다 |
...
> **합의 이후 바뀐 부분이다.** 원래는 "스트릭을 건드리지 않는다" 였다.

A 를 그대로 두면 **올려서 스트릭만 챙긴 뒤 바로 지우는 것**이 가능하다 — 피드에 아무것도 안 내놓고 기록만 가져간다. 그래서 **그날 것을 내리면 스트릭에서 뺀다.**

```text
오늘 것을 내림   → 그날을 빼고 다시 센다. 다시 올리면 복구된다
지난 글을 내림   → 스트릭 그대로
```

**과거로 소급하지 않는 것이 요점이다.** 30일 연속 중 5일째를 내렸다고 그 지점에서 끊으면 며칠치가 한꺼번에 사라진다. 그런 삭제는 **남용이 아니다** — 스트릭은 이미 며칠 전에 받았고 지워서 새로 얻는 것이 없다. 남용은 당일에만 성립한다.

그래서 재계산 조회도 **지우는 그 한 건만 뺀다**(`findApprovedDatesInRoundExcept`). 이미 내려간 다른 행들은 그대로 센다 — `deleted_at IS NULL` 을 붙이면 과거 삭제까지 소급 반영된다.

원래 A 를 고른 근거 중 하나가 *"되돌리기 로직이 어렵다"* 였는데, **`#155` 가 `MemberChallenge#recalculateStreak` 를 만들면서 그 근거는 사라졌다.** 1 을 빼거나 날짜를 되돌리는 것이 아니라 남은 인증으로 처음부터 센다.

> **재화 회수는 별개다.** 인증 통과 시 지급하기로 하면서, 글을 내리면 그 지급분을 회수한다.
> 잔액이 모자라면 삭제 자체를 거부한다 — 부채를 만들면 갚을 수단이 없어지기 때문이다.
> 자세한 것은 재화 이슈에서 다룬다.
...
> ⚠️ **기간은 보류 상한보다 길어야 한다.** 심사가 장애로 판정을 못 하면 보류하기로 하면서(위 [심사가 장애로 답을 못 주면 보류한다](#심사가-장애로-답을-못-주면-보류한다--설계안-합의-필요)) **대기본이 보류 기간 동안 살아 있어야 한다.** 상한 24시간에 수명 주기 3일이 그 조합이다. **1일로 먼저 걸어 두고 나중에 늘리면 그 사이 보류 건이 지워진다.**
