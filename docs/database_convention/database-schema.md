...
| 코드값 | 시안 표기 | 성격 | 얻는 법 |
| --- | --- | --- | --- |
| `GEM` | 파란 보석 | **무료 재화** | 챌린지 인증 리워드 |
| `TOPAZ` | 주황 보석 ("토파즈", 충전 팝업에서는 "코인") | **유료 재화** | 현금 결제 · 보너스 |
...
> ⚠️ **이 표는 한동안 반대로 적혀 있었다.** `GEM` 을 "현금 결제", `TOPAZ` 를 "`GEM` 으로 교환"으로 적었는데 기획 확인 결과 뒤집혔다. 지갑을 만들 때는 어느 쪽이 무엇인지 정해지지 않아 두 재화를 대칭으로 두었고, 그 사이 추정으로 채운 값이 그대로 남아 있었다. **`Currency.java` 주석도 같이 틀려 있었다.**

**유료 전용·무료 전용 아이템이 갈린다.** 무료 재화로만 살 수 있는 것이 있고, 유료 재화로만 살 수 있는 것이 따로 있다.

> ⚠️ **그런데 지금 지갑은 그 구분을 두 축에 중복 표현하고 있다** — 재화 종류(`GEM`/`TOPAZ`)와 잔액(`free_balance`/`paid_balance`)이다. 어느 축이 진실인지 확정되기 전까지 `MemberWallet.deduct()` 의 차감 순서와 잔액 응답 형태는 **미결이다.** 자세한 내용과 선택지는 지갑 모델 이슈에 있다.
...
#### 차감은 무상부터 한다 — **현재 구현이며, 확정된 정책이 아니다**

지금 `MemberWallet.deduct()` 는 **`free_balance` 를 먼저 쓰고, 모자라면 `paid_balance` 에서 뺀다.** 그 근거는 이랬다.

- 유상 잔액을 남겨 두어야 환불 요구에 그대로 응할 수 있다
- 무상에는 소멸 정책이 붙을 수 있으므로 먼저 쓰는 것이 사용자에게 유리하다

> ⚠️ **이 근거는 무상·유상을 "같은 재화의 회계 구분"으로 볼 때만 성립한다.** 둘이 서로 다른 구매력이라면(무료 전용·유료 전용 아이템이 갈린다면) 흘러넘침 자체가 성립하지 않는다 — 무상 결제가 유료 재화를 소모하게 되기 때문이다. 위 [재화는 두 종류다] 의 경고와 같은 뿌리이고, **어느 축이 진실인지 정해지기 전까지 이 절은 확정 규칙이 아니라 현재 동작의 기록이다.**
...
잔액   5                     →  삭제 거부(409). 5 가 더 필요하다고 알린다
```

**부채를 만들지 않기 위해서다.** 미상환을 남기면 그만큼 제재를 걸어야 하는데, 제재 중에는 인증을 못 해 갚을 수단이 없다 — 빠져나올 수 없는 상태가 된다.

> ⚠️ **여기서 말하는 "잔액"이 무엇인지는 아직 미결이다.** 지금 구현은 유상·무상을 합한 값으로 재는데, 무료 재화와 유료 재화가 서로 다른 구매력이라면 무상만으로 재야 한다. 합계로 재면 **유상 잔액이 있는 사용자에게는 거부가 발동하지 않고, 회수가 현금으로 산 재화를 깎는다.** 위 [재화는 두 종류다] 의 경고와 같은 뿌리다.
