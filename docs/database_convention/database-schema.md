...
| idempotency_key | VARCHAR(150) | N | **멱등 키.** 유니크 |
| reference_type | VARCHAR(30) | Y | 근거가 된 대상의 종류(예: 인증·아이템·충전) |
| reference_id | BIGINT | Y | 그 대상의 id |
| created_at / updated_at | DATETIME(6) | N / N | 생성·수정 시각 |

- `UNIQUE(idempotency_key)` — **같은 요청이 두 번 들어와도 두 번 반영되지 않는다.** 네트워크가 끊겨 클라이언트가 재시도하는 것은 정상 동작이고, 그때 두 번 차감되거나 두 번 지급되면 그대로 돈 문제가 된다.
