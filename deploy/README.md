...
> ⚠️ **이 파일과 콘솔이 지금 다르다. 그대로 붙여넣지 말 것.**
>
> | | 콘솔 | 이 파일 |
> | --- | --- | --- |
> | `challenge-verifications-staging/*` 삭제 | **있다** | 이번에 맞췄다 |
> | `chat-emoticons/*` 열거·삭제 | 없다 | 있다 |
>
> **`-staging` 삭제는 지금 실제로 쓰이는 권한이다** — 심사를 통과한 사진을 공개 prefix 로 복사한 뒤 대기본을 지운다. 파일에 빠져 있어 이번에 더했다. 반대로 **이모티콘 쪽은 파일이 앞서 나가 있다**: 정리 배치가 `media.cleanup.enabled = false` 로 꺼져 있어 아직 필요 없다. **배치를 켜는 시점에 콘솔을 넓힌다** — 순서가 뒤바뀌면 매일 `AccessDenied` 로그만 쌓인다.
