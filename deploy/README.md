...
> **`AI_REVIEW_MAX_IMAGE_DIMENSION` 은 성능 설정이 아니라 비용 설정이다.** 심사기가 받은 이미지를 자기 쪽에서 줄여 주지 않으므로, 줄이는 주체는 우리뿐이고 보낸 크기가 그대로 토큰이 된다. 예전 값(1568)은 정반대 근거로 정해져 있었다 — 그때 쓰던 심사기가 어차피 축소해서 봤기 때문에 우리 축소는 전송 속도만을 위한 것이었다. 낮출수록 싸지지만, 작게 찍힌 신분증 같은 것을 놓치면 유해 판정이 헐거워진다. 비용만 보고 내리지 말고 실측으로 정한다.
