...
> **`repair`는 이력 테이블만 고친다.** 실제 스키마는 건드리지 않는다. "파일을 고쳤으니 그 변경을 DB에 반영"해 주지 않으므로, 스키마를 바꾸려면 새 `V__`를 추가해야 한다.
...
| 어디서 | 무엇을 보는가 |
| --- | --- |
| 배포 Discord 알림 | `ENV_FILE` 시크릿에 `JPA_DDL_AUTO`가 있으면 경고 한 줄 추가. 배포 성패와 무관하게 붙는다 |
