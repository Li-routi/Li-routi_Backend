...
사진이 그 챌린지의 의도에 맞는지 OpenAI 에게 물어 통과 여부를 정한다. 맞지 않으면 **422 로 반려하고 저장하지 않는다** — 스트릭도 오르지 않는다.

### 필수 환경변수 — 없으면 앱이 부팅하지 못한다

| 키 | 필수 | 기본 | 설명 |
| --- | --- | --- | --- |
| `OPENAI_API_KEY` | ✅ | 없음 | **기본값을 두지 않았다.** 키 없이 배포되는 것을 부팅 실패로 막는다 |
| `AI_REVIEW_ENABLED` | | `true` | 끄면 자가인증으로 되돌아간다 |
| `AI_REVIEW_MODEL` | | `gpt-5.6-luna` | |
| `AI_REVIEW_TIMEOUT` | | `PT20S` | 인증 요청 하나가 이만큼 늦어질 수 있다 |
| `AI_REVIEW_MAX_IMAGE_DIMENSION` | | `1024` | AI 에 보내기 전 줄일 긴 변 픽셀. **이 값이 곧 이미지 비용이다** — 아래 참고 |

배포 워크플로가 `OPENAI_API_KEY` 존재를 **시작 전에** 확인한다. 값 자체는 비밀이라 로그에 찍지 않고 있는지만 본다.

> **`AI_REVIEW_MAX_IMAGE_DIMENSION` 은 성능 설정이 아니라 비용 설정이다.** 보낸 크기가 그대로 토큰이 되므로 낮출수록 싸지지만, 작게 찍힌 신분증 같은 것을 놓치면 유해 판정이 헐거워진다. 비용만 보고 내리지 말고 실측으로 정한다. 자세한 근거는 [아래](#이미지는-줄여서-보낸다)와 `AiReviewProperties.maxImageDimension` 에 있다.
>
> ⚠️ **이 값은 `docker-compose.prod.yml` 에도 기본값이 있고 그쪽이 이긴다.** `.env` 에 변수가 없으면 compose 기본값이 그대로 들어가므로, `application.yaml` 만 고치면 배포에는 아무 효과가 없다. 두 값은 항상 같이 옮긴다.
...
```bash
docker compose exec -T app printenv OPENAI_API_KEY >/dev/null && echo "키 주입됨" || echo "키 없음"
```
...
막는 쪽으로 두면 심사기가 죽는 순간 아무도 인증을 못 한다. 그때 어차피 킬 스위치를 켜서 통과시키게 되므로, 처음부터 통과시키고 로그로 드러내는 편이 정직하다. 부적절한 사진의 방어선이 이것 하나가 아니라는 점도 근거다 — 신고가 임계값만큼 쌓이면 전체 회원에게 가려진다.
...
원본은 S3 에 그대로 두고, **AI 에 보낼 때만** 긴 변을 `1024px` 로 줄인다. 심사 쪽이 받은 이미지를 자기 쪽에서 줄여 주지 않으므로 **보낸 크기가 그대로 비용**이다 — 줄이는 주체가 우리뿐이다. 값과 그 근거는 `AiReviewProperties.maxImageDimension` 에 있다.

> 예전 값은 `1568px` 이었고 근거가 정반대였다 — 그때 쓰던 심사기가 그보다 큰 이미지를 어차피 자기가 축소해서 봤기 때문에, 우리 축소는 전송 속도만을 위한 것이었다. 제공자를 바꾸면서 전제가 사라졌다.
