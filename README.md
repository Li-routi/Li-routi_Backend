...
### 5. 스웨거로 API 확인하기 (개발용 토큰)

인증이 필요한 API를 스웨거에서 눌러보려면 JWT가 필요한데, 로컬에서는 받을 방법이 마땅치 않습니다. 소셜 로그인은 카카오·구글 서버에 실제 토큰을 검증하러 가고, 배포 서버에서 받은 토큰은 서명 키가 환경마다 달라 로컬에서 안 통합니다.

그래서 **로컬에서만 존재하는** 발급 경로를 둡니다.

```bash
curl -X POST http://localhost:8080/api/dev/token/9001
```

응답의 `accessToken`을 스웨거 우측 상단 **Authorize** 에 넣으면 됩니다. 만료는 14일이라 한 번 받아두면 한동안 다시 받지 않아도 됩니다.

- 로컬 더미 회원은 **9001**, **9002** 입니다 (`db/dummy`).
- 없는 회원이거나 탈퇴한 회원이면 404 입니다.

> 이 엔드포인트는 `local` 프로파일에서만 등록되고, 배포 산출물(`bootJar`)에서는 클래스 자체가 빠집니다. 스웨거가 공개 경로라 운영에 열리면 누구나 임의 회원의 토큰을 받을 수 있기 때문입니다.

### 6. 커밋 시 자동 검사 (pre-commit)
