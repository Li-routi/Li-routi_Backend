...
/**
 * 통합 테스트가 만든 회원의 업적·스트릭·활동일·캐릭터 종속 행을 회원 삭제 전에 정리한다.
 *
...
public final class MemberFixtureCleanup {
    ...
    public static void deleteDependencies(EntityManager entityManager, Long memberId) {
        ...
        entityManager.createNativeQuery("delete from member_activity_day where member_id = :memberId")
                .setParameter("memberId", memberId)
                .executeUpdate();
        entityManager.createNativeQuery("delete from pending_popup where member_id = :memberId")
                .setParameter("memberId", memberId)
                .executeUpdate();
        entityManager.createNativeQuery("delete from member_selected_character where member_id = :memberId")
                .setParameter("memberId", memberId)
                .executeUpdate();
        entityManager.createNativeQuery("delete from member_character where member_id = :memberId")
                .setParameter("memberId", memberId)
                .executeUpdate();
    }
    ...
    public static void deleteDependencies(JdbcTemplate jdbcTemplate, Long memberId) {
        ...
        jdbcTemplate.update("delete from pending_popup where member_id = ?", memberId);
        jdbcTemplate.update("delete from member_selected_character where member_id = ?", memberId);
        jdbcTemplate.update("delete from member_character where member_id = ?", memberId);
    }
}
