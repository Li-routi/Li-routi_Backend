...
public class NotificationCreationService {
    ...
    private final NotificationSettingPolicy settingPolicy;

    /**
     * 새로 만든 경우에만 그 ID를 돌려주고, 사용자 설정이 꺼졌거나 이미 같은 사건 알림이
     * 있었으면 null을 돌려준다(비활성 설정과 재시도·중복 이벤트는 Push도 보내지 않아야 하므로
     * 호출부가 이 null로 배송을 건너뛴다). DB의 충돌 무시 insert(ON DUPLICATE KEY UPDATE)로 처리하므로,
    ...
    public Long create(Long memberId, NotificationCategory category, NotificationType type,
        ...
        if (!settingPolicy.allows(memberId, type)) {
            return null;
        }
