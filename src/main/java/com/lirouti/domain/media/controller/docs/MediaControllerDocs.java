package com.lirouti.domain.media.controller.docs;
...
public interface MediaControllerDocs {
    ...
    @ApiResponses({
            ...
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "400",
                    description = "지원하지 않는 형식이거나 해당 용도에서 허용하지 않는 형식"
            ),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "401",
    ...
    ApiResponse<MediaResDTO.PresignedUrl> issuePresignedUrl(MediaReqDTO.PresignedUrl request);
}
