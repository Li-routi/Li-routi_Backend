...
--
-- ⚠️ 이 표는 이 파일이 소유한다. character_unlock_condition 을 V__ 마이그레이션에서 UPDATE 해도
--    소용이 없다 — 반복 마이그레이션은 V__ 가 전부 끝난 뒤에 돌고, 여기 upsert 가 그 값을 도로
--    덮는다. 실제로 V20260818142647 이 파도를 ACH-EG-013 으로 옮겼는데 이 파일이 계속 되돌려,
--    파도의 도전 업적에 알이 붙지 않았다. 매핑을 바꾸려면 반드시 이 파일을 고친다.
--
-- 해금은 업적으로만 한다.
--
...
VALUES
    ...
    -- 파도 — 파도의 도전
    --
    -- ⚠️ ACH-EG-006(취미의 물결) 이 아니다. V20260818142647 이 파도를 새 업적으로 옮겼다.
    --    achievement_reward_item 에는 아직 옛 매핑(EG-006 → 파도의 알)이 남아 있으니 그것을
    --    근거로 삼지 말 것 — 그 표는 코드가 읽지 않아 갱신되지 않은 채 남아 있다.
    (7, 8, 'ACHIEVEMENT_CLAIMED', 'ACH-EG-013', 1, 1, NOW(6), NOW(6)),
    -- 까루 — 알림 보고 왔어요
